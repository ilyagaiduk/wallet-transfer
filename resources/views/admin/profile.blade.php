    ...
    <aside class="main-sidebar sidebar-dark-primary elevation-4">
        ...
        <div class="sidebar">
            ...
            <div class="user-panel mt-3 pb-3 mb-3 d-flex">
                ...
                <div class="info">
                    <a href="" class="d-block">Пользователь {{$user->name}}</a>
                </div>
            </div>
            <!-- Sidebar Menu -->
            <nav class="mt-2">
    ...
    <section class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Профиль пользователя</h1>
        ...
        <section class="content">
            <div class="container-fluid">
                <h3>Перевести деньги пользователю</h3>
                <form method="post" action="{{route('operation.transfer')}}">
                    @csrf
                    <div class="mb-2">
                    <label for="id">Выберите пользователя</label>
                    <select name="id" id="id">
                        @foreach($usersForTransfer as $value)
                    ...
                    <div class="mb-2">
                    <label for="amount">Какую сумму перевести</label>
                    <input type="text" name="amount" id="amount">
                    </div>
                    <input type="hidden" value="{{$user->id}}" name="source">
                    <div class="mb-2">
                        <input class="btn-success btn-sm" type="submit" value="Перевести">
                    </div>
                </form>
                <h3>Пополнить кошелек пользователя</h3>
                <form method="post" action="{{route('operation.refill')}}">
                    @csrf
                    <div class="mb-2">
                        <label for="id">Выберите пользователя</label>
                        <select name="id" id="id">
                ...
                <h3>Снять деньги с текущего кошелька</h3>
                <form method="post" action="{{route('operation.withdrawal')}}">
                    @csrf
                    <div class="mb-2">
                    </div>
                    <div class="mb-2">
                        <label for="amount">Какую сумму снять</label>
                        <input type="text" name="amount" id="amount">
                    </div>
                    <input type="hidden" value="{{$user->id}}" name="source">
                    <div class="mb-2">
                        <input class="btn-success btn-sm" type="submit" value="Снять деньги">
                    </div>
                </form>
                @if ($errors->any())
